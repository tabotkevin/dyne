...
class Backend(ABC):
    ...
    async def authorize(self, required_role, user) -> bool:
        """
        ...
        provided = await self.get_user_roles_callback(user)
...
class DigestAuth(Backend):
    ...
    def _parse_credentials(self, credentials: str) -> dict:
        digest_directive_re = re.compile(r'\s*([^=,\s]+)\s*=\s*"?([^",]*)"?')
        return {k: v for k, v in digest_directive_re.findall(credentials)}

    async def authenticate(self, request):

        if not hasattr(request, "session"):
            raise RuntimeError("DigestAuth requires session middleware to be enabled.")
    ...
    def login_required(self, f=None, role=None, optional=None):
        ...
        def decorator(f):
            ...
            async def decorated(request, response, *args, **kwargs):
                # Try to find a matching backend based on the scheme
                selected = None
                ...
                # If no compatible scheme found, use the first one as the 401 challenger
                if not selected:
